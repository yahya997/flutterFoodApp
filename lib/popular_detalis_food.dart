...
class PopularDetailsFood extends StatefulWidget {
  PopularModel popularModel;

  PopularDetailsFood(this.popularModel);

  @override
  _PopularDetailsFoodState createState() => _PopularDetailsFoodState();
}

class _PopularDetailsFoodState extends State<PopularDetailsFood> {
              ...
              background: Container(
                decoration: BoxDecoration(
                  image: DecorationImage(
                      image: NetworkImage(widget.popularModel.image),
                      fit: BoxFit.cover),
                ),
              ),
            ),
            ...
            InkWell(
              onTap: () {
                ...
                documentReference.setData({
                  'user_id': userId,
                  'food_id': widget.popularModel.id,
                  'name': widget.popularModel.name,
                  'image': widget.popularModel.image,
                  'price': widget.popularModel.price,
                  'priceTotal': _getPrice(),
                  'discount': widget.popularModel.discount,
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            Text(
              widget.popularModel.name.toUpperCase(),
              style: TextStyle(fontSize: 22, letterSpacing: 1.2, height: 1.25),
            ),
            SizedBox(height: 10),
            Text('\u0024 ${widget.popularModel.price}',
  ...
  String _getPrice() {
    double price =
    ((double.parse(widget.popularModel.price) * quantityValue.toDouble()));
        ...
        builder: (BuildContext bc) {
          return StreamBuilder(
            stream: Firestore.instance
                .collection('Comment')
                .where('food_id', isEqualTo: widget.popularModel.id)
                .snapshots(),
            builder:
